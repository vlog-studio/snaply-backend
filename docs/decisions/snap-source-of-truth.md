...
> 2026-08-11. 스냅(3~5초 원본 클립)의 원천을 디바이스 로컬에서 **서버로 전환**하기로 결정하고,
> 그 근거(업계 조사), 단계별 이행 계획, 스토리지 용량 정책을 기록한다.
> 스키마/API 변경의 기준 문서. 관련: [plan-limits.md](./plan-limits.md), [api-spec.md](../api-spec.md)
>
> **정책은 결정됐고 구현은 아직 없다.** §4 스키마·§5 로드맵·§6 쿼터 집행은 모두 제안 상태이며,
> 남은 판단은 [backlog.md](../backlog.md) A-4(§7 미결 사항을 옮긴 것)에 있다.
...
Standard/Premium 한도는 **미확정**. 참고 제안: Standard(₩9,900) 100GB — 실저장 원가
월 ~₩4,900(스토리지만, 여유 충분), Premium(₩24,900) 500GB — 월 ~₩24,500로 스토리지만으로
마진이 얇아지므로 300GB 안도 검토. 크레딧(무비 과금) 기획과 함께 확정할 것.
...
이 결정에서 남은 판단(위치 정보 서버 저장 여부, 비로그인 사용자의 스냅 지위, 삭제 유예 기간 값,
egress 실측 후 렌디션 다운로드 정책, 앱의 해상도 하드코딩 해소)은
[backlog.md](../backlog.md) **A-4** 로 옮겼다. Standard/Premium 스토리지 한도와
크레딧 기반 무비 결제 설계는 플랜 정책과 함께 정해야 하므로 **A-2** 에 있다.

미결 항목을 이 문서에 다시 만들지 말 것 — 두 곳에 있으면 하나를 닫아도 나머지가 낡는다.
