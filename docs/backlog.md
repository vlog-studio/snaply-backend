...
> 마지막 정리: 2026-09-09 — 생애주기 결정 3축과 무비 세부 규칙 5개가 닫히면서 A-1 을 남은
> 구현 항목으로 다시 썼고, 완료된 E-6(낡은 Prisma 클라이언트 프리체크)을 내렸다.
...
- [ ] **`서버`** **SNS 게시 시 자동 끝내기** — 게시가 성공하면 `Movie.resultVideoId` 로 무비를 찾아
      끝내기 처리한다. 이 경로는 서버가 성공을 알고 있어 추측이 아니다
- [ ] **`서버`** **e2e 실검증** — `media:e2e` 를 무비 경로로 갱신
...
- [ ] **`서버`** **만료 예고 알림**(SNAP-13) — **서버 FCM 파이프라인 확장이 선행**이다
      (현재 geofence 전용, 무비 완료 알림도 여기 얹힌다). 화면의 남은 기간 표시는 위 `앱` 항목
- [ ] **무비가 참조 중인 스냅의 만료 예외 여부**(정책) — 프로젝트는 영구 보존인데 스냅은 15일이라,
      몇 달 뒤 무비를 열면 컷이 대부분 비어 있을 수 있다. 예외를 두지 않으면 프로젝트 보존의
      의미가 옅어진다(다시 만들 재료가 없다). 앱 전환 설계와 함께 판단한다
