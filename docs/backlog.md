...
**광고 보상 — 값 확정됨(2026-08-18), 남은 것은 쿨다운뿐**: 지급 경로·검증 규칙·정책 값은
[decisions/ad-reward-credits.md](./decisions/ad-reward-credits.md)에서 확정됐고 구현·테스트가
끝났다. 실제 지급은 여전히 C-6(AdMob 콘솔 설정)에 막혀 있다.
- **1회 보상 크레딧 20 · 일일 한도 5 — 확정**(같은 문서 §7). 20 × 5 = export 1편이라
  **둘을 따로 바꾸지 않는다**. 기준 시각도 KST 자정으로 확정됐다
- **쿨다운 길이**만 미결이다 (현재 300초, 클라이언트 UX 검토 후). 세션 TTL 은 쿨다운을
  넘길 수 없으므로 함께 본다
- 원가·eCPM **실측은 아직 없다**. 파일럿에서 광고 순매출이 20원을 밑돌면 한도가 아니라
  **보상량을 먼저 조정한다** — 한도 인하는 되돌릴 수 없는 혜택 축소다
