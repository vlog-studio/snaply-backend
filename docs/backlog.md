...
- [ ] **`서버`** **무비 완료 알림의 FCM 전환** — 지금은 앱의 로컬 알림이다. 완료를 아는 것은
      Python 워커인데 FCM 은 Node 에 있어 **발송 주체를 먼저 정해야 한다**. 워커에 심으면
      서비스 계정이 두 서비스로 갈라지고 조용한 시간대·알림 설정 판정이 두 언어로 중복된다 —
      워커가 API 를 부르거나 API 가 큐 완료를 듣는 쪽이 낫다
