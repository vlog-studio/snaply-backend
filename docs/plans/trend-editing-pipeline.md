...
5. `+faststart` · CRF · `loudnorm` · `sidechaincompress` — 반나절
6. **ASS 애니메이션 자막 + 한글 폰트** — 새 라이브러리 0, 체감 효과 1위
7. **VAD 기반 무음 자동 컷** — 이미 있는 whisper 로 가능. "AI 자동 편집"이 여기서 처음 사실이 된다
8. 비트 그리드 스냅 컷 (`snapToBeat`)
9. **MediaPipe 얼굴 앵커**(크기·회전 포함) + A/B 폴백 체인 + 세이프 에어리어 규칙(§5.2)
10. **손글씨 팩 1종(약 20에셋) + 모션 프리셋 3종 + 원격 매니페스트 + 삭제 로깅**(§8)

### 2단계

11. 단일 `filter_complex` 통합 (재인코딩 3회 → 1회)
12. 알파 오버레이 트랙 베이킹
13. 텍스트 배지(ASS 인프라 공유) · 스티커 팩 2·3종 추가
14. `xfade` 종류 확장 + 줌 펀치 · 속도 램프

### 3단계

15. 제품 앵커(vision 확장 → 필요 시 Florence-2) · 추적 스무딩 · 삭제율 기반 자동 은퇴
16. 오토리프레임 · 안정화 · NVENC
