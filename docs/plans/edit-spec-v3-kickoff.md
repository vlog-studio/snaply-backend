...
| # | 내용 | 왜 이 순서인가 |
|---|---|---|
| 1 | 어휘 사전과 빌드 컨텍스트 (§3) | 사전 없이는 스펙·매니페스트의 `anchor` 절을 쓸 수 없다 |
| 2 | 스테이지별 `attempt` 와 해시 고정 (§4) | 무효화 표의 한 열이 여기서 나온다 |
| 3 | 무효화 규칙 (§5) | A-2·A-5·B-6 를 한 사전이 흡수한다 |
| 4 | 잔여 개정 (§6) | 위 셋이 정해진 뒤 기계적으로 반영된다 |
...
- **[trend-editing-pipeline.md](./trend-editing-pipeline.md) §3** — "레이어는 타임라인에 붙는다 /
  키프레임은 절대 시각(ms)" 문장을 B-7 결정에 맞춰 수정. **안 고치면 두 문서가 상반된 원칙을
  선언한 채 남는다**
