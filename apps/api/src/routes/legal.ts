 ...
 *  2. **보유 기간.** 국외 이전 표의 "해당 사업자의 정책에 따름" 은 확인 전의 자리 표시다.
 *     실제 보관 기간(남용 모니터링 목적 보관 포함)과 학습 이용 여부를 확인해 숫자로 적을 것.
 *  3. **국외 이전 표의 나머지 수탁자.** 지금은 리포지터리에서 확인 가능한 두 곳(OpenAI·Supabase)만
 *     적었다. AWS·Firebase·RevenueCat·Sentry 의 리전을 확인해 채울 것.
        ...
        `<h2>1. 수집하는 정보</h2>
         ...
         <table>
           ...
           <tr><td>OpenAI</td><td>미국</td><td>영상에서 추출한 정지 이미지 최대 4장</td>
               <td>영상 내용 분석</td><td>분석 완료 후 해당 사업자의 정책에 따름</td></tr>
