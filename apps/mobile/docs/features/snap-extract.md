...
| --- | --- | --- |
| Entry points | `Functional` | The dashed `가져오기` cell that leads the newest day's row of the snaps tab grid (`widgets/snap-grid/ui/snap-import-cell.tsx`) — kept even when the library is empty, and disabled (not hidden) during selection mode. There is no header action and no separate empty-state button (see [Snap library](snaps.md)). It opens the system photo picker first and navigates only when a video was chosen, so backing out of the picker goes nowhere. |
