...
| Saving a snap | `Functional` | On capture completion the temporary recording is moved into the app document directory, its real length and display size are read back from it (`shared/lib/video-metadata`, after the move rather than beside it — reading a file being relocated is not a race worth taking; the size comes rotation-applied from the native probe in `shared/lib/video-trim`, so a phone held upright is stored upright), and a snap is created from all of it. When the file cannot be read, the snap keeps the requested length and an upright 1080×1920 stand-in, both unmarked, for the metadata backfill to measure later. Owned by `features/capture-moment`. Nothing else happens — the snap joins no movie. |
...
| Select a previous recording | `Functional` | The in-screen recording library (찍은 스냅 · 앱에 저장된 스냅 N개, opened from the 찍은 스냅 N개 보기 button; 아직 찍은 스냅이 없어요 when empty) can select (enters a preview/review stage) or delete a locally stored original, after a 스냅을 삭제할까요? / 삭제한 스냅은 되돌릴 수 없어요. alert. Deleting goes through `features/delete-snap`, so an original already used by a movie is removed from it too (see [Snap library](snaps.md#deleting-an-original)). |
| Web recording | `Prototype` | The shutter is disabled and the UI explains that recording requires iOS or Android. |

The page uses four explicit stages: `idle`, `recording`, `saving`, and `review`, presented over a near-black surface with mono overlays (a live "스냅 N개" counter, the "꾹 눌러 찍기" hint, an ember REC dot, a "스냅을 저장하는 중…" badge while saving, and a brief "담김 · 스냅 N개" badge after each capture) and an ember shutter. Recording runs only while the shutter is held, and a progress ring around it (`ui/hold-ring.tsx`, react-native-svg stroke-dashoffset driven by Reanimated) fills linearly over the selected duration, rewinding over 250ms on release. When the OS "reduce motion" setting is on, the ring shows a static partial arc instead of a continuous fill. The accidental-tap threshold is owned by `model/hold-gesture.ts`. The `review` stage is reached only by selecting a recording from the library, not by capturing. Capture, save, and recording-library failures are displayed inside the camera screen and can be dismissed (스냅을 저장하지 못했어요. 다시 시도해 주세요. · 찍은 스냅을 불러오지 못했어요.).

Closing the recorder mid-recording stops the camera and discards the in-flight snap; nothing is persisted. The on-screen countdown (`3초`, `2초`, …) is display-only — the actual stop is driven by the native `maxDuration`, so after the counter reaches zero the badge shows a finishing state until the recording completes.

The camera-permission-denied screen (스냅을 찍으려면 카메라 권한이 필요해요.) still exposes the recording library, so users can browse, select (which enters the review stage), and delete saved originals without camera permission.
