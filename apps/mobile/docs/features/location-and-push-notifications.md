...
| Present a foreground push | `Partial` | A message arriving while the app is foregrounded is re-presented as a local notification (`expo-notifications`), because FCM suppresses the system banner in the foreground. Its `data` travels with it, so a tap on the re-presented copy routes exactly like a tap on the original. Requires the `expo-notifications` native module to be present in the build. |
| Open a notification's target | `Functional` (unverified on device) | Tapping a notification takes the user where it points instead of merely opening the app: a `movie_ready` or `movie_failed` payload opens that movie (`movieHref`), a `snap_expiry` payload opens the snap library tab — the destination an expiry notice exists to reach (SNAP-13). Both channels are covered (an OS-displayed push through the FCM adapter, an app-presented notice through the local adapter), and so is the tap that launched the app from a quit state, which is honoured once the root navigator exists and the user is signed in. One tap reported twice — by two channels, or by the opening query and the live listener — is acted on once. A payload with no `kind` this build knows opens the app and nothing more. |
...
| `src/_app/providers` | `notification-tap-router.tsx`, `notification-target.ts` | Where a tapped notification goes. `notificationTarget` is the pure map from a payload's `kind` (the server's vocabulary plus the app's own `movie_failed`) to an href; `NotificationTapRouter` subscribes to both tap channels once, asks each once for the launch tap, waits for the navigator and the sign-in before navigating, and de-duplicates one tap reported twice. Lives here because it spans two features' destinations. |
| `src/_app/providers` | `app-providers.tsx` | Mounts `PushTokenGate`, `NotificationTapRouter`, `GeofenceGate`, and `MovieGenerationBridge`, alongside the five headless nodes this feature does not use ([Application shell and navigation](app-shell-and-navigation.md#composition-and-ownership)). |
| `src/_app/providers` | `movie-generation-bridge.tsx` | The same bridge shape for the other notification preference: reads `useMovieReadyEnabled` and lets `compose-movie` announce a **failed** generation — a completed one is the server's `movie_ready` push (2026-09-12), which this preference does not yet reach (root backlog B-6). Documented in [The movie screen](movie.md). |
