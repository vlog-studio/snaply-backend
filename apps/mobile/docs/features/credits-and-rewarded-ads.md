...
The SDK is wired and the Android app builds with it, but **no ad has been shown yet**: `react-native-google-mobile-ads` is installed, its config plugin puts the Android App ID in the merged manifest, and `admobRewardAdProvider` is the provider the flow selects whenever a real API origin is configured. What no one has seen is an ad actually playing and a credit actually landing.
...
- **No credit has ever been earned end to end.** An ad plays on device, but the reward has never been granted — detailed in [Required to finish](#required-to-finish-admob-integration). No consent flow (UMP/ATT) either, and iOS is unconfigured.
