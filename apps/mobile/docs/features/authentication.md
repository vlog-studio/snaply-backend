...
| Sign in with email and password | `Functional` | The email form validates the address format and password presence through `emailSignInSchema` (React Hook Form + Zod), then runs `supabase.auth.signInWithPassword`. A not-yet-confirmed account surfaces a distinct message pointing the user to the confirmation link ("이메일 인증이 아직 안 됐어요. 가입할 때 받은 메일의 링크를 눌러 주세요."). The screen's hero reads "3초씩 찍으면\n한 편의 무비가 돼요", with 비밀번호를 잊었나요? and 계정이 없나요? 가입하기 under the form and the disclaimer "계속하면 서비스 이용약관과 개인정보 처리방침에 동의하게 돼요." Requires configured Supabase credentials (see below). |
...
/sign-in → "비밀번호를 잊었나요?" → /reset-password → resetPasswordForEmail(redirectTo) → "메일 확인" notice
...
- Google sign-in is offered; Apple sign-in is deferred (code/metadata retained, not listed in `socialProviders`). Offering social login on iOS eventually requires Apple sign-in for App Store review, so enable Apple before an iOS store submission. Kakao/Naver are not offered. Kakao is planned for after the service is complete ([backlog A-8](../../../../docs/backlog.md#a-8-카카오-로그인--서비스-완성-후-인증-추가-계획)): Supabase has a built-in Kakao provider, but it always requests the `account_email` consent item, which requires a Kakao Biz App. Naver has no built-in provider.
