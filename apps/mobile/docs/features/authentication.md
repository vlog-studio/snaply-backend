...
Users sign in to Snaply with an email and password, with Google, or — in builds that enable it — with Kakao, before reaching the app: they can create an account (confirmed via an emailed link that deep-links back into the app), sign in, and reset a forgotten password (also via an emailed link). A signed-in session gates the main experience and persists across restarts, and users can sign out from Settings. Google social sign-in is offered on the sign-in screen, Kakao is offered ahead of it where `EXPO_PUBLIC_KAKAO_SIGN_IN_ENABLED=true`, and Apple sign-in is deferred — its code and metadata are retained but not offered until the provider is configured.
...
| Sign in with Google | `Functional` | The sign-in screen renders `SocialLoginList` below the email form. Tapping it runs the PKCE OAuth flow (`signInWithOAuth` → in-app browser consent → `exchangeCodeForSession`); the resulting user is mirrored into the session and the guard reveals the app. Requires the Google provider configured in Supabase (see below). |
| Sign in with Kakao | `Partial` | Implemented in the app, not yet verified end to end: the Kakao console and the Supabase provider are not set up (2026-09-24). With `EXPO_PUBLIC_KAKAO_SIGN_IN_ENABLED=true` the list shows a **카카오 로그인** button first — Kakao's design guide fixes its label, its #FEE500 container, black symbol, and black-at-85% label — and it runs the same PKCE flow as Google with `provider: 'kakao'`. Without the flag the button is absent, since pressing it could only fail with "provider is not enabled". The user's display name comes from the Kakao nickname (Supabase maps it to `full_name`/`name`). |
...
| Email sign-in action + provider abstraction; Google and Kakao social sign-in action, provider metadata (`socialProviders`, built by `offeredSocialProviders` from `KAKAO_SIGN_IN_ENABLED` in `src/shared/config/auth.ts`), and button UI | `src/features/sign-in` |
...
- Google/Apple/Kakao consoles: register the Supabase callback `https://<project-ref>.supabase.co/auth/v1/callback`.
- **Kakao** (before setting `EXPO_PUBLIC_KAKAO_SIGN_IN_ENABLED=true` in a build):
  - Kakao Developers: create the app; under App Settings → Platform Key take the **REST API key** (the Client ID) and issue and activate the **Client Secret**; register the callback above as the Kakao Login Redirect URI; set Kakao Login to ON.
  - Consent items: `profile_nickname` and `profile_image`, plus `account_email`. Supabase's Kakao provider **always requests `account_email`** and offers no way to drop it, so the item must exist or Kakao rejects the request (KOE205). `account_email` is only available to a **Biz App** (business registration, or the identity-verified individual-developer Biz App), so this is a prerequisite, not an option. Set it as optional consent unless the product needs email.
  - Supabase → Auth → Providers → Kakao: enable it with the Client ID and Client Secret, and turn on **Allow users without an email** so a user who declines the optional email consent can still sign in.
  - Accounts are linked by Supabase only through a matching verified email: a person who signs in once with Google and once with Kakao without sharing an email gets two separate accounts.
...
- Google sign-in is offered; Apple sign-in is deferred (code/metadata retained, not listed in `socialProviders`). Offering social login on iOS eventually requires Apple sign-in for App Store review, so enable Apple before an iOS store submission. Kakao is implemented but off by default (`EXPO_PUBLIC_KAKAO_SIGN_IN_ENABLED`), pending the console setup above. Naver is not offered — Supabase Auth has no built-in Naver provider.
- The Google and Kakao OAuth flows use the `expo-web-browser` / `expo-auth-session` native modules and the `snaplyapp://` custom scheme, so they need a development build or standalone app (not the standard Expo Go client), on the same device that initiates it (PKCE stores the code verifier locally).
