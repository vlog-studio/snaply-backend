...
- Google sign-in is offered; Apple sign-in is deferred (code/metadata retained, not listed in `socialProviders`). Offering social login on iOS eventually requires Apple sign-in for App Store review, so enable Apple before an iOS store submission. Kakao/Naver are still not offered. Supabase Auth has a built-in Kakao provider (`provider: 'kakao'`, configured with the Kakao app's REST API key and client secret), so Kakao needs no custom OIDC setup; Naver has no built-in provider.
