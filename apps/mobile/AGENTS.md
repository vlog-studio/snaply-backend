...
| Calling the backend, or reacting to a change in its contract (`@vlog-studio/shared-types`) | [`docs/workflows/api-contract-integration.md`](docs/workflows/api-contract-integration.md) | Sections | How `apiRequest` derives its typing from the shared Zod contract, tooling, layered data flow, and the Zod boundary policy |
...
| Running, testing, or verifying changes on a device or simulator | [`docs/workflows/local-development-and-testing.md`](docs/workflows/local-development-and-testing.md) | Sections | Verification surfaces (automated → simulator/emulator → owner's device for device-only behavior); macOS environment constraints; iOS Simulator via Expo Go, Android via dev build (Expo Go no longer boots this app on Android), and EAS Build |
| Driving a physical Android device the owner has attached over wireless adb (optional; the simulator/emulator is the default surface) | [`docs/workflows/android-device-verification.md`](docs/workflows/android-device-verification.md) | Sections | Wireless-adb toolkit: screenshots, UI hierarchy, input injection, app-private state, logs, permissions, and the limits of each |
